...
adjusted for github.com/openshift/ci-tools-standalone alongside main.go.
...
func makeIssue(repo string, number int, title string) github.Issue {
	return github.Issue{
		HTMLURL: fmt.Sprintf("fake://localhost/o/%s/pull/%d", repo, number),
		Title:   title,
	}
}
...
func TestRun(t *testing.T) {
	...
	manyComments := []int{}
	for i := 0; i < 100; i++ {
		manyIssues = append(manyIssues, makeIssue("r", i, "many "+strconv.Itoa(i)))
	...
	cases := []struct {
		...
		},
		{
			...
			client: fakeClient{issues: []github.Issue{
				makeIssue("r", 1, "problematic this should work"),
				makeIssue("error", 2, "problematic expect an error"),
				makeIssue("r", 3, "problematic works as well"),
			}},
			err:      true,
			expected: []int{1, 3},
		},
